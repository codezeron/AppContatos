...
class ContactFormViewModel(
    ...
    fun onPhoneChanged(value: String) {
        ...
        if (newPhone.length <= 11 && uiState.formState.phoneNumber.value != newPhone) {
            uiState = uiState.copy(
                formState = uiState.formState.copy(
                    phoneNumber = uiState.formState.phoneNumber.copy(
                        value = newPhone,
                        errorMessage = validatePhone(newPhone)
                    )
                )
            )
        }
    ...
    private fun validatePhone(phone: String): String {
        return if (phone.isNotBlank() && (phone.length < 10 || phone.length > 11)) {
            "Telefone inválido"
        } else {
            ""
        }
    ...
    private fun isValidForm(): Boolean {
        uiState = uiState.copy(
            formState = uiState.formState.copy(
                ...
                phoneNumber = uiState.formState.phoneNumber.copy(
                    errorMessage = validatePhone(uiState.formState.phoneNumber.value)
                ),
                email = uiState.formState.email.copy(
